...
dataroot = r"C:\Users\YcDen\Documents\projectG\data\CICIDS2017.csv"
...
x_train, x_test, y_train, y_test = train_test_split(
    Data_X, Data_Y, test_size=0.3, random_state=42, stratify=Data_Y
)
...
for epoch in range(EPOCH):
    for step, (b_x, b_y) in enumerate(trainloader):  # gives batch data
        b_x = b_x.view(
            -1, 1, INPUT_SIZE
        ).float()  # reshape x to (batch,time_step,input_size)
        b_x = b_x.to(device)
        b_y = b_y.to(device)
...
torch.save(rnn.state_dict(), r'C:\Users\YcDen\Documents\projectG\modelLSTM.pth')